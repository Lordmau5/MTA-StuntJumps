...
-- WIP fancy UI
function MainUIClass:onStart()
	local width, height = guiGetScreenSize()
	local uiWidth, uiHeight = 800 / 2560 * width, 500 / 1440 * height

	self.ui.window =
		dgsCreateWindow(100, 100, uiWidth, uiHeight, "Stunt Jumps", false, self.textColor, 25, nil, self.titleColor)
	...
	self.ui.checkbox["30FPSLimit"] =
		dgsCreateCheckBox(0.02, 0.02, 0.1, 0.05, "30 FPS Limit", false, true, self.ui.tab["settings"].tab)
	self.ui.checkbox["canFallOffBike"] =
		dgsCreateCheckBox(0.02, 0.07, 0.1, 0.05, "Can Fall Off Bike", false, true, self.ui.tab["settings"].tab)
	self.ui.checkbox["drawBoundingBoxes"] =
		dgsCreateCheckBox(0.02, 0.12, 0.1, 0.05, "Draw Bounding Boxes", false, true, self.ui.tab["settings"].tab)
...
function MainUIClass:buildJumpPacks()
	...
	self.ui.tab["jumps"].updateInfo = function()
		...
		dgsSetText(
			jumpCompleted,
			"Completed: " .. tostring(Completions:getPackCompletions(pack)) .. " / " .. tostring(pack:getCount())
		)
	end
...
function MainUIClass:getSelectedJumpPackItem()
	return dgsGridListGetSelectedItem(self.ui.tab["jumps"].list)
end

function MainUIClass:getSelectedJumpPack(item)
...
function MainUIClass:updateJumpsTab()
	...
	for name, pack in pairs(StuntJumps:getAll()) do
		repeat
			...
			dgsGridListSetItemText(
				self.ui.tab["jumps"].list,
				row,
				self.ui.tab["jumps"].columnActive,
...
function MainUIClass:updateCheckboxes()
